"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BusinessHourService = void 0;
const mongoose_1 = require("@nestjs/mongoose");
const common_1 = require("@nestjs/common");
const mongoose_2 = require("mongoose");
let BusinessHourService = class BusinessHourService {
    constructor(businessHourModel) {
        this.businessHourModel = businessHourModel;
    }
    async getAll() {
        return await this.businessHourModel.find().exec();
    }
    async getById(id) {
        return await this.businessHourModel.findById(id).exec();
    }
    async create(businessHour) {
        const createdDelivery = new this.businessHourModel(businessHour);
        return await createdDelivery.save();
    }
    async update(id, businessHour) {
        return await this.businessHourModel.findByIdAndUpdate(id, businessHour, { new: true });
    }
    async delete(id) {
        return await this.businessHourModel.deleteOne({ _id: id }).exec();
    }
};
BusinessHourService = __decorate([
    common_1.Injectable(),
    __param(0, mongoose_1.InjectModel('BusinessHour')),
    __metadata("design:paramtypes", [mongoose_2.Model])
], BusinessHourService);
exports.BusinessHourService = BusinessHourService;
//# sourceMappingURL=business-hour.service.js.map